...
const createUser = asyncHandler(async (req, res) => {
  try {
    ...
    if (!findEmail) {
      ...
      // Create a new user with the reset token
...
const UpdateUser = asyncHandler(async (req, res) => {
  ...
  validMongoDbId(_id);
  try {
    const updatedduser = await User.findByIdAndUpdate(
      _id,
      {
...
const changePassword = asyncHandler(async (req, res) => {
  ...
  if (!(await findUser?.isPasswordMatched(old_password))) {
    ...
    (await findUser?.isPasswordMatched(old_password)) &&
    (await findUser?.isPasswordMatched(new_password))
  ) {
    return res.status(401).json({
      message: "New password not equal to old password",
      status: false,
    });
  }
  // Update the password
